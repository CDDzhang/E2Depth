...
import torch
import numpy as np
# SynchronizedSceneDataset 是 DGP 官方工具库提供的数据加载器，能同时加载多个模态（相机+激光雷达）
from dgp.datasets.synchronized_dataset import SynchronizedSceneDataset
# Camera Pose 用于相机模型和位姿转换
    ...
    if len(sample) == 1:
        ...
        for key in sample:
            if key in ['idx', 'dataset_idx', 'sensor_name', 'filename']:
                continue
            if is_tensor(sample[key]):
                sample[key] = sample[key].unsqueeze(0)
            elif is_numpy(sample[key]):
                sample[key] = np.expand_dims(sample[key], 0)
            elif is_list(sample[key]):
                sample[key] = [
                    v.unsqueeze(0) if is_tensor(v) else
                    np.expand_dims(v, 0) if is_numpy(v) else v
                    for v in sample[key]
                ]
        return sample
...
"""
DATUMS = ['CAMERA_%02d' % idx for idx in [1, 5, 6, 7, 8, 9]] + ['lidar']
"""
class DGPDataset:
    def __init__(self, 
                 path,  # str, Path to the dataset
                 split, # str {'train', 'val', 'test'}, Which dataset split to use
                 cameras=None,  # list of str, Which cameras to get information from
                 depth_type=None,  # str, 使用lidar数据生成深度图
                 input_depth_type=None,
                 with_pose=False,   # If enabled pose estimates are also returned
                 with_semantic=False,  # If enabled semantic estimates are also returned
                 back_context=0,  # Size of the backward context(前n帧)
                 forward_context=0,  #  Size of the forward context(后n帧)
                 data_transform=None,  # Transformations applied to the sample
                 ):
        self.path = path
        self.split = split
        self.dataset_idx = 0
        ...
        self.with_depth = depth_type is not None and depth_type != ''
        self.with_pose = with_pose
        self.with_semantic = with_semantic

        self.input_depth_type = input_depth_type
        self.with_input_depth = input_depth_type is not None and input_depth_type != ''
    ...
    def generate_depth_map(self, sample_idx, datum_idx, filename):
        ...
        用 LiDAR 点云投影到相机，生成相机视角下的深度图，并缓存结果。
        depth : np.array [H, W]
        """
