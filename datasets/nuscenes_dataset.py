...
from datasets.data_utils import img_loader, mask_loader_scene, align_dataset, transform_mask_sample
from datasets.dgp_dataset import stack_sample
...
class NuScenesdataset(Dataset):
    ...
    def __init__(self, path, split,
        ...
        with open('datasets/nuscenes/{}.txt'.format(self.split), 'r') as f:
    ...
    def get_current(self, key, cam_sample):
        ...
        # get current timestamp camera extrinsics
            ...
            T_world_ego = self.get_tranformation_mat(ego_pose)
            T_ego_cam = self.get_tranformation_mat(cam_param)
            T_world_cam = T_world_ego @ T_ego_cam
            return T_world_cam.astype(np.float32)
        else:
    ...
    def generate_depth_map(self, sample, sensor, cam_sample):
        ...
        filename = '{}/{}'.format(
                        os.path.join(self.path, 'samples'),
                        'DEPTH_MAP/{}/{}'.format(sensor, cam_sample['filename'].split("/")[-1]))
        filename = filename.replace(".jpg", ".npz")
        ...
        if os.path.exists(filename):
            ...
            ego_lidar_points += lidar_to_ego_translation   # 机体系中的雷达坐标
            ...
            ego_pose = self.dataset.get(
                    'ego_pose', cam_sample['ego_pose_token'])   # 世界系中的相机姿态
            ...
            # Ego -> sensor
            sensor_sample = self.dataset.get(
                'calibrated_sensor', cam_sample['calibrated_sensor_token'])  # ego->sensor
    ...
    def get_tranformation_mat(self, pose):
        ...
        T[:3, :3] = Quaternion(pose['rotation']).rotation_matrix
        T[:3, 3] = np.array(pose['translation'], dtype=np.float64)
        return T.astype(np.float32) # 4 * 4
        ...
        for cam in self.cameras:
            ...
            if self.has_context:
                ...
                if self.with_pose:
                    ...
                    data.update({
                        # 其实外参是完全一致的，不需要怎么变化
                        'pose_bwd': pose_context[0] if self.bwd else None,
                        'pose_fwd': pose_context[1] if self.fwd and self.bwd else (
                                     pose_context[0] if self.fwd and not self.bwd else None)
                    })
