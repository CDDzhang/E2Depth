...
def align_dataset(sample, scales, contexts, with_pose=False):
    """
    ...
    org_images = sample['rgb'] if 'rgb_original' not in sample else sample['rgb_original']
    org_contexts= sample['rgb_context'] if 'rgb_context_original' not in sample else sample['rgb_context_original']
    ...
    for scale in scales: 
        ...
        sample[('inv_K', scale)]= np.linalg.inv(scaled_K).copy()
