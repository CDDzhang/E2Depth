...
from utils.misc import pretty_ts
from utils.depth import cal_depth_error
...
class Logger:
    ...
    def __init__(self, cfg, use_tb=True):
        ...
        log_file = os.path.join(self.log_path, "train_log.txt")
        # 如果已存在则在末尾添加 _1, _2, ...
        if os.path.exists(log_file):
            idx = 1
            while True:
                new_name = f"train_log_{idx}.txt"
                new_path = os.path.join(self.log_path, new_name)
        ...
        print(f"create save log file: {log_file}")
        if use_tb: 
            self.init_tb()
    ...
    def log_time(self, epoch, batch_idx, duration, loss, start_time):
        ...
        log_str = (
            ...
            f"loss: {rep_loss:.3f} | "
            f"time elapsed: {pretty_ts(time_sofar)}"
        )
        print(log_str)
    ...
    def compute_depth_losses(self, inputs, outputs, vis_scale=False):
        ...
        for cam in range(self.num_cams):
            ...
            cam_abs_rel[cam] = depth_errors_metric[0].item()
        ...
        return error_metric_dict, error_median_dict, cam_abs_rel        
