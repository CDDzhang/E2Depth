...
from losses.loss_util import compute_photometric_loss, compute_edg_smooth_loss, compute_masked_loss, compute_auto_masks
...
class SingleCamLoss(BaseLoss):
    ...
    def compute_reproj_loss(self, inputs, target_view, cam=0, scale=0, ref_mask=None):
        ...
        for frame_id in self.frame_ids[1:]: 
            reproj_loss_args = {
                'pred': target_view[('color', frame_id, scale)],  # 网络预测图像
                'target': inputs['color',0, 0][:, cam, ...]  #  相机采集图像  
            }
            reprojection_losses.append(
                compute_photometric_loss(**reproj_loss_args)  # 计算两帧间的photometric_loss，分别表示-1,0 和 0,1的
            )                
