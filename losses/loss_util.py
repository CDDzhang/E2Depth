...
def compute_ssim_loss(pred, target):
    ...
    """
    ref_pad = torch.nn.ReflectionPad2d(1)  # 镜像反射图像边界填充
    pred = ref_pad(pred)
    target = ref_pad(target)

    mu_pred = F.avg_pool2d(pred, kernel_size = 3, stride = 1)   # 均值
    mu_target = F.avg_pool2d(target, kernel_size = 3, stride = 1)

    musq_pred = mu_pred.pow(2)  # 方差
    musq_target = mu_target.pow(2)
    mu_pred_target = mu_pred*mu_target  # 均方差
